...
%   30Aprsl18: allow for limit on number of individual patch spectra to plot
...
for ids=1:length(data_sources)
    if ~isempty(data_sources{ids})
        disp(sprintf('%1.0f ->%6s data from %s',ids,data_sources{ids}.type,data_sources{ids}.label));
    end
end
ifok=0;
while ifok==0
...
switch data_source
    ...
    case {2,3} %ffdm, but rois already cut into squares and in mat-files
        ...
        if (data_source==2) %acmd: 2 -> direct from patch database (read from patch_metadata), 
            ...
            [sdb,if_ok,sdb_warnings,sdb_warnings_file,subinfo]=mlis_alglib_sdbcheck(sdb,check_filenums,check_if_ask,check_if_verbose,subinfo,prefix);
            %
            sdba=[];
...
%        preprocess_downsample=1; set by default, so removed 27Jun22
    ...
    case 6 %natural images from Penn database
        ...
        if_ok=0;
        while (if_ok==0) %N and R must be powers of 2
...
%        preprocess_downsample=1;% set by default, so removed 27Jun22
        ...
        ifok=0;
        while (ifok==0)
            regtype_count=getinp('number of region types','d',[1 5]);
            regtype_masks=cell(regtype_count,1);
            regtype_descs=cell(regtype_count,1);
...
disp(sprintf('Minimum roi size: %4.0f x %4.0f (i.e., all subjects and all views have an ROI this size or larger)',roi_minsize));
%
...
%
n_patches=floor(roi_sizes(:,:,1)/NR_max).*floor(roi_sizes(:,:,2)/NR_max);
n_patches_min=min(n_patches(:));
n_patches_max=max(n_patches(:));
disp(sprintf('There are %4.0f to %4.0f patches of size %4.0f x %4.0f pixels per image',n_patches_min,n_patches_max,NR_max,NR_max));
if ~exist('patches_usefrac') patches_usefrac=1; end
patches_usefrac=getinp('fraction of patches to use (1: use all)','d',[1 n_patches_max],1);
...
%for data_source=4 (usic), there may be more than one roi for each subject and view, and we need to divide it up into patches
%for data_source=2, 3, 5, or 6, there are multiple patches for each subject and view already, and no need to divide up
%
if data_sources{data_source}.if_makepatches %data_source = 1 or 4
    %choose image patches by tesselating them into size NR_max x NR_max, with a random start point for each row and column of
    %patches (given by patch_strips), and then choosing up to max_patches randomly from them
    %
    ...
    for isubj=1:count_subjs
        for iview=1:count_views
            ...
            for iexamp=1:nexamps
                ...
                patch_count_thisexamp=min(ceil(n_patches(isubj,iview)/patches_usefrac),max_patches);
                if (if_norand==1)
                    patch_choices=find([1:n_patches(isubj,iview)]<=patch_count_thisexamp);
                else
                    patch_choices=find(randperm(n_patches(isubj,iview))<=patch_count_thisexamp);
                end
                ...
                patch_starts{isubj,iview}=cat(1,patch_starts{isubj,iview},...
                    [patch_strips{isubj,iview,1}(patch_x)',patch_strips{isubj,iview,2}(patch_y)' repmat(iexamp,patch_count_thisexamp,1)]);
            end %iexamp
        end %iview
    end %isubj
else %datasource=2, 3, 5, or 6
    ...
    elseif (data_source==4)
        ...
        if length(examp_index)~=1
            ...
            disp(sprintf(' example is index %5.0f, taken from subj ptr %3.0f (orig subj %3.0f), view ptr %2.0f (orig view %2.0f), patch %3.0f',...
                ...
                file_nos(examp_subj(iexamp)),examp_view(iexamp),....
...
%                         rect_mask_composite=getfield(getfield(nibs_get_patchpair(nibs_bpdata,nibs_aspect_ptr,nibs_smoothness_ptr,ipatch,...
%                             setfields(opts_nibs_patchpair,{'if_noread','img_size'},{1,nibs_patches_img_size(ipatch,:)})),'rect_mask'),'composite');
%                         mask=ones(size(rect_mask_composite)); %assume nothing is to be used
%                         for im=1:length(regtype_masks{iregtype}); %set to zero any pixel to use 
%                             mask(rect_mask_composite==regtype_masks{iregtype}(im))=0;
%                         end
%                         rect_mask_sampled=mask(Rsample{ixs},Rsample{iys});
%                         rect_mask_scaled=squeeze(mean(mean(reshape(rect_mask_sampled,[S,size(patch_sampled,1)/S,S,size(patch_sampled,2)/S]),3),1));
%                         rect_mask_scaled=(rect_mask_scaled~=0); %any nonzero pixel means do not use
%                         %apply mask before binarizing
                        [rect_mask_composite,rect_mask_sampled,rect_mask_scaled]=...
                            nibs_get_mask(nibs_bpdata,nibs_aspect_ptr,nibs_smoothness_ptr,ipatch,nibs_patches_img_size(ipatch,:),nibs_NRpatch_max,...
                            regtype_masks{iregtype},setfields(opts_ds,{'xs','ys','S'},{Rsample{ixs},Rsample{iys},S}));
                        median_cut=median(patch_scaled(rect_mask_scaled(:)==0));
                    else
 ...
 %                   patches_binarized{iNR}{ixs,iys}(:,:,ipatch,iregtype)=uint8(patch_scaled>median(patch_scaled(:)));
    ...
    for iregtype=1:regtype_count
        ...
        for ipatch=1:patch_count_sum
            ...
            for ixs=1:nstep
                for iys=1:nstep
                    patch=patches_binarized{iNR}{ixs,iys}(:,:,ipatch,iregtype);
...
counts_per_patch=zeros(1,length(N_list));
std_counts=zeros(length(N_list),count_views,btc_avg_types); %expected standard dev based on counts
%need to do this for each regtion type (masks already applied)
for iNR=1:length(N_list)
    N=N_list(iNR);
    NR=NR_list(iNR);
    R=NR/N;
    ...
    std_counts(iNR,:,1)=sqrt(1./counts_per_patch(iNR)); %variance of a btc statistic  calculated from one patch
    for iview=1:count_views
        std_counts(iNR,iview,2)=sqrt(1./counts_per_patch(iNR)*mean(1./(patch_count(s_havedata{iview},iview)*patch_mult(iNR)))); %patch_mult is patch_mult_side.^2
        for isubj=1:count_subjs
            btc_avg_vs(iNR,:,iview,isubj)=mean(patches_btc{iNR}(:,find((patches_view{iNR}==iview)&(patches_subj{iNR}==isubj))),2);
        end
        %patch weighted
        btc_nus_v(iNR,iview,1)=sum(patches_view{iNR}==iview);
        btc_avg_v(iNR,:,iview,1)=mean(patches_btc{iNR}(:,find(patches_view{iNR}==iview)),2);
        btc_std_v(iNR,:,iview,1)=std(patches_btc{iNR}(:,find(patches_view{iNR}==iview)),0,2);
        %subject weigted
        btc_nus_v(iNR,iview,2)=sum(s_havedata{iview}>0); %changed from count_subjs, 04Dec19
        btc_avg_v(iNR,:,iview,2)=mean(btc_avg_vs(iNR,:,iview,s_havedata{iview}),4);
        btc_std_v(iNR,:,iview,2)=std(btc_avg_vs(iNR,:,iview,s_havedata{iview}),0,4); %normalize by count-1
        %drop-one quantities
        for isubj=1:count_subjs
            %subj_list=setdiff([1:count_subjs],isubj);
            subj_list=setdiff(s_havedata{iview},isubj);
            %weight patches equally
            btc_avg_v_drop(iNR,:,iview,1,isubj)=mean(patches_btc{iNR}(:,find((patches_view{iNR}==iview)&(patches_subj{iNR}~=isubj))),2);
            btc_std_v_drop(iNR,:,iview,1,isubj)=std(patches_btc{iNR}(:,find((patches_view{iNR}==iview)&(patches_subj{iNR}~=isubj))),0,2);
            %weight subjects equally
            btc_avg_v_drop(iNR,:,iview,2,isubj)=mean(btc_avg_vs(iNR,:,iview,subj_list),4);
            btc_std_v_drop(iNR,:,iview,2,isubj)=std(btc_avg_vs(iNR,:,iview,subj_list),0,4); %normalize by count-1          
        end
        %
    end
    %across views
    btc_avg_allv(iNR,:,:)=mean(btc_avg_v(iNR,:,:,:),3);
    btc_std_allv(iNR,:,1)=std(patches_btc{iNR},0,2);
    %take into account that not every subject has every view
    %previous: btc_std_allv(iNR,:,2)=std(reshape(btc_avg_vs(iNR,:,:,:),[1 btc_n,count_views*count_subjs]),0,3);
    btc_avg_vs_allviews=zeros(1,btc_n,1,0);
    for iview=1:count_views
    	subj_list=setdiff(s_havedata{iview},isubj);
        btc_avg_vs_allviews=cat(4,btc_avg_vs_allviews,btc_avg_vs(iNR,:,iview,s_havedata{iview}));
    end
    btc_std_allv(iNR,:,2)=std(btc_avg_vs_allviews,0,4);
    %drop-one quantities
    ...
    for isubj=1:count_subjs
        ...
        btc_avg_vs_allviews_drop=zeros(1,btc_n,1,0);
        for iview=1:count_views
            subj_list=setdiff(s_havedata{iview},isubj);
            btc_avg_vs_allviews_drop=cat(4,btc_avg_vs_allviews_drop,btc_avg_vs(iNR,:,iview,subj_list));
        end
        btc_std_allv_drop(iNR,:,2,isubj)=std(btc_avg_vs_allviews_drop,0,4);
    end
    %
end
...
for btc_avg_type=1:btc_avg_types %1->patch weighted, 2->subject weighted
    ...
    for iview=1:count_views
        disp(sprintf(' view %5s: %4.0f examples',view_names{view_nos(iview)},btc_nus_v(iNR,iview,btc_avg_type)));
        disp(btc_header);
        for iNR=1:length(N_list);
            N=N_list(iNR);
            NR=NR_list(iNR);
    ...
    disp(sprintf(' average across all views: %4.0f examples',sum(btc_nus_v(iNR,:,btc_avg_type))));
    disp(btc_header);
    for iNR=1:length(N_list);
        N=N_list(iNR);
        NR=NR_list(iNR);
    ...
    disp(sprintf(' standard dev across all views: %4.0f examples',sum(btc_nus_v(iNR,:,btc_avg_type))));
    disp(btc_header);
    for iNR=1:length(N_list);
        N=N_list(iNR);
        NR=NR_list(iNR);
