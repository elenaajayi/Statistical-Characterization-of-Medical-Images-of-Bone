...
%to do list:
%more images
%maybe weighted average so each image counts the same amount?
%
...
while (i_patch~=0)
    ...
    if (i_patch>0)
        ...
        title_string=sprintf(' patch %3.0f from roi %3.0f from image %s',i_patch,i_roi,im_file);       
        figure;
...
%
pad_factor=getinp('padding factor for Fourier Transform','d',[0 1],1);
nfft=patch_size*(1+pad_factor);
...
tick_locs=zerofreq+(1+pad_factor)*patch_size/2*tick_fracs;
...
drawnow;
%
%show power spectrum as functoin of frequency magnitude
%
